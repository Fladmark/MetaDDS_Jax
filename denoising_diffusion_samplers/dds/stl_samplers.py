...
#
#     def init_sample(self, n, key):
#         return torch.normal(0, self.sigma, (n, self.dim))
#
#     def f_aug(self, y, t, args, detach=False):
#         t_ = t * torch.ones((y.shape[0], 1), dtype=self.dtype)
#
...
#         gamma_t_sq = self.g_aug(y, t, args)[..., :self.dim] ** 2
#
#         u_t_normsq = ((u_t - b_t) ** 2 / gamma_t_sq).sum(dim=-1)[..., None] / 2.0
#
#         n, _ = y_no_aug.shape
#         zeros = torch.zeros((n, 1), dtype=self.dtype)
#
#         state = torch.cat((u_t, zeros, u_t_normsq), dim=-1)
#         return state
#
#     def g_aug(self, y, t, args):
#         t_ = t * torch.ones((y.shape[0], 1), dtype=self.dtype)
...
#         n, _ = y_no_aug.shape
#
#         gamma_t = torch.sqrt(self.gamma) * torch.ones_like(y_no_aug)
#
#         zeros = torch.zeros((n, 1), dtype=self.dtype)
#
#         b_t = self.prior_drift(y_no_aug, t_)
...
#         delta_t = (u_t - b_t) / gamma_t
#
#         out = torch.cat((gamma_t, delta_t, zeros), dim=-1)
#
#         return out
#
#
...
#         #y_no_aug = y[..., :self.dim]
#         # REPLACE
#         y_no_aug = y[..., :self.dim].detach().numpy().astype(np.uint32)
#
#
#
#         n, _ = y_no_aug.shape
...
#
#         # REMOVE
#         key = key.detach().numpy().astype(np.uint32)
#
#
#         param_trajectory, ts = integrator(
#             self.dim, self.alpha, self.f_aug, self.g_aug, y0_aug, key, dt=dt,
